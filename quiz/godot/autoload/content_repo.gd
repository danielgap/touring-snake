...
func _load_minigames_from(path: String) -> Array[MiniGame]:
	...
	if not FileAccess.file_exists(path):
		last_error = "Minigames file not found: %s" % path
		emit_signal("content_missing", path)
		emit_signal("minigames_loaded", 0)
		return minigames
	...
	var file: FileAccess = FileAccess.open(path, FileAccess.READ)
	if file == null:
		last_error = "Unable to open minigames file: %s" % path
		emit_signal("content_error", last_error)
		emit_signal("minigames_loaded", 0)
		return minigames
	...
	var parsed: Variant = JSON.parse_string(raw_text)
	if typeof(parsed) != TYPE_DICTIONARY:
		last_error = "Minigames JSON must be an object with 'minijuegos' array"
		emit_signal("content_error", last_error)
		emit_signal("minigames_loaded", 0)
		return minigames

	var raw_array: Variant = parsed.get("minijuegos", [])
	if typeof(raw_array) != TYPE_ARRAY:
		last_error = "Minigames JSON 'minijuegos' must be an array"
		emit_signal("content_error", last_error)
		emit_signal("minigames_loaded", 0)
		return minigames
