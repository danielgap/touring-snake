...
func _draw() -> void:
	...
	var radius: float = (_grid.CELL_SIZE / 2.0 - 2.0) * scale
	draw_circle(screen_pos, radius + 8.0, Color(UiLab.ACCENT_MAGENTA, 0.12))
	draw_circle(screen_pos, radius + 4.0, Color(ITEM_COLOR, 0.28))
